...
### 做法
DP 
$dp[i][j] = max(dp[i][1 ... j - 1], dp[i - 1][j - 1] + i * a[i])$
...
首先 $BFS$ 两次算出树的直径两点，第一次 $BFS$ 随机开始点，求出据随机点最远的点 $D1$ ，这就是直径的一个端点，第二次从这个点开始 $BFS$ ，求出另一个直径端点 $D2$
...
### 题意
给一无向图，把边染成两种颜色  
$c1$ 只考虑红色边，并计算图中连接组件的数量
$c2$ 只考虑蓝色边，并计算图中连接组件的数量
最小化 $c1 + c2$
