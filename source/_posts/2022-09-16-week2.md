...
### 题意
圆盘周围坐着 $n$ 个人。圆盘上有 $n$ 个菜，各有编号 $Ci$。圆盘可以随意转动，要求最小化 $\sum_1^n(dis(i, Ci))$
...
### 做法
如果 $X$ 的人名的前缀字符串不是一个人名（$Y$ 的），那就是 $n/2$，否则 $Y$ 将一直在前面，$X$ 名次向后  
使用 $Trie$ 来保存人名词典
...
### 题意
$v(x, y)$ 是字符串 $S$ 的从下表 $x$ 到 $y$ 的切片转为数字的函数，给定 $li$、$ri$、$w$，使得 $(v(L1, L1 + w - 1) * v(li, ri) + v(L2, L2 + w - 1)) modulo9 = k$，求 $L1$、$L2$，且最小化

### 做法
$modulo9$ 使得转数字函数原本 $*10$ 进位可以变成加法，这样使用前缀和可以轻松求得片段和
